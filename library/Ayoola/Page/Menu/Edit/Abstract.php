...
abstract class Ayoola_Page_Menu_Edit_Abstract extends Ayoola_Page_Menu_Abstract
{
	...
	public function createForm( $submitValue, $legend = null, Array $values = null )
    {
		...
		if( @$values['sub_menu_name'] || is_array( $this->getGlobalValue( 'link_options' ) ) && in_array( 'sub_menu', $this->getGlobalValue( 'link_options' ) ) )
		{
			...
			$fieldset->addElement( array( 'name' => 'sub_menu_name', 'label' => 'Please select (<a rel="spotLight" href="' . Ayoola_Application::getUrlPrefix() . '/object/name/Ayoola_Page_Menu_List/">Manage Menu</a>)', 'type' => 'Select', 'value' => @$values['sub_menu_name'] ), $options );
			$fieldset->addRequirement( 'sub_menu_name', array( 'InArray' => array_keys( $options )  ) );
			$form->addFieldset( $fieldset );
		}
/* 		else
		{
