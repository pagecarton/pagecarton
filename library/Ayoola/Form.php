 ...
 */	//	echo $this->getForm();
	...
	//	var_export( $this->_filters[$name] );
		...
		foreach( $this->_filters[$name] as $seive => $parameter )
		{
			...
			if( is_array( $this->_values[$this->_names[$name]['real_name']] ) )
			{
    ...
    protected function _validate( $name )
    {
		...
		if( ! Ayoola_Form::hasPriviledge() )
		{
			...
			{
				if( $each != strip_tags( $each, '<span> <div> <a> <address> <em> <strong> <b> <i> <big> <small> <sub> <sup> <cite> <code> <img> <ul> <ol> <li> <dl> <lh> <dt> <dd> <br> <p> <table> <th> <td> <tr> <pre> <blockquote> <nowiki> <h1> <h2> <h3> <h4> <h5> <h6> <hr>' ) )
				{
    ...
    public function getRequiredFieldset()
    {
		...
		//	var_export( $this->submitValue );
			$element->addElement( array( 'name' => 'submit-' . $this->submitValue . '', 'value' => $this->submitValue, 'type' => 'Submit', 'style' => 'display:block;' ) );
		}
		...
		$element->addElement( array( 'name' => $this->_attributes['name'] . self::SUBMIT_DETECTOR, 'value' => $this->_attributes['name'], 'type' => 'Hidden' ) );
		$element->addElement( array( 'name' => 'MAX_FILE_SIZE', 'type' => 'Hidden', 'value' => '107374182' ) );
		$element->addElement( array( 'name' => self::HONEY_POT, 'type' => 'HoneyPot' ) );
 ...
 *///		
		...
		//	var_export( $each );
			...
			//		$class = new $class( @self::$_requirementOptions[$combinedInfo]['parameters'] );
				...
				//	var_export( $combinedInfo );
					...
					{
						$fieldset->appendElement = false;
						$fieldset->addLegend(  $combinedInfo['requirement_legend'] . ( $fieldset->getLegend() ?  ' | ' . $fieldset->getLegend() : null ) );
	...
	//	if( is_array( $_POST['fake_values_namespace'][$this->formNamespace] ) )
		...
		if( $this->oneFieldSetAtATime && isset( $_POST['Ayoola_Form'][$this->formNamespace]['one_field_set_at_a_time'] ) )
		{
 ...
 */			//	$group = $fieldset->group;
		...
		//	var_export( self::getStorage( 'global_values' )->retrieve() );
			...
			{
				if( $this->oneFieldSetAtATime && ! isset( $_REQUEST[self::BACKBUTTON_INDICATOR] ) && ! isset( $_REQUEST[self::REFRESH_INDICATOR] ) )
				{
					//	var_export( $fieldsetId );
					//	var_export( $i );
...
//		if( ! $this->oneFieldSetAtATime || ! $values['oneFieldSetAtATime'] )
	...
	//	self::v( $values );
		...
		if( $this->isSubmitted() )
		{
			...
			if( $this->oneFieldSetAtATime &&  ! $this->_badnews )
			{ 
	...
	//	self::getStorage( $this->_attributes['id'] )->clear();
		...
		//	$form .= '<button rel="ignore" onclick="var result=confirm( \'Do you want to go back to the previous stage?\' );result?this.setAttribute( \'rel\', \'\' ):null; return result;" type="submit" name="' . self::BACKBUTTON_INDICATOR . '" value="<"><img height="25" src="/img/backbutton.png" title="Go back to previous form fields" alt="<"></button>';
			$form .= '<input class="badnews" rel="ignore" onclick="var result=confirm( \'Do you want to go back to the previous stage?\' );result?this.setAttribute( \'rel\', \'\' ):null; return result;" type="submit" title="Go back to previous form fields" name="' . self::BACKBUTTON_INDICATOR . '" value=" Back ">';
		...
		//	$form .= '<button rel="ignore" onclick="var result=confirm( \'Do you want to start all over?\' );result?this.setAttribute( \'rel\', \'\' ):null; return result;" type="submit" name="' . self::REFRESH_INDICATOR . '" value="o"><img height="25" src="/img/reload.png" title="Start form all over" alt="o"></button>';
			$form .= '<input class="badnews" rel="ignore" onclick="var result=confirm( \'Do you want to start all over?\' );result?this.setAttribute( \'rel\', \'\' ):null; return result;" type="submit" name="' . self::REFRESH_INDICATOR . '" value=" Start all over " title="Start form all over"/>';
		}
			//		var_export( $values );
	...
	//		$form .= "<div style='display:inline;'>\n";
		...
		//		$form .= "<span style='display:inline-block;' class='badnews'>{$this->_badnews[$name]}</span>\n";
			...
			//	self::$_checkValues = true;
				$replace = "<span style='display:block;' class='badnews'>{$this->_badnews[$name]}</span>\n";

			}
			$markup = str_ireplace( self::$_placeholders['badnews'], $replace, $markup );
			$form .= $markup;
//			$form .= "</div>\n";    
		}
		$form .= $fieldset->allowDuplication ? "<button class='boxednews  ' type='button' title='" . ( @$fieldset->duplicationData['add'] ? : "Duplicate this fieldset" ) . "' onClick='try{ ayoola.xmlHttp.callAfterStateChangeCallbacks(); }catch( e ){}var fieldset = this.parentNode.cloneNode( true ); var fieldtags= [ \"input\", \"textarea\", \"select\"]; for ( var tagi= fieldtags.length; tagi-->0; ) { var fields = fieldset.getElementsByTagName( fieldtags[tagi] ); for( var i = fields.length; i-->0; ){ fields[i].value= \"\"; } } this.parentNode.parentNode.insertBefore( fieldset, this.parentNode.nextSibling ); ayoola.xmlHttp.callAfterStateChangeCallbacks(); this.name=\"\" '>" . ( @$fieldset->duplicationData['add'] ? : "+" ) . "</button>\n" : null; 
		$form .= $fieldset->allowDuplication ? "<button class='boxednews' type='button' title='" . ( @$fieldset->duplicationData['add'] ? : "Remove this fieldset" ) . "' onClick='confirm( \"Delete all the elements in these fieldset?\") ? this.parentNode.parentNode.removeChild( this.parentNode ) : null;'>" . ( @$fieldset->duplicationData['remove'] ? : "-" ) . "</button>\n" : null; 
	//	var_export( $div );
	//	var_export( $this->_values );
...
//		var_export( $_REQUEST[$name] );
	...
	//	if( $name == 'page_options' )
		...
		if( isset( $values[$name] ) && is_null( $defaultValue ) )
		{
