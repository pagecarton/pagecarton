...
class Ayoola_Object_Creator extends Ayoola_Object_Abstract
{
    ...
    {
	//	var_export( __LINE__ );
		$this->createForm( 'Create Object', 'Create a new viewable Object' );
		$this->setViewContent( $this->getForm()->view(), true );
		if( ! $values = $this->getForm()->getValues() ){ return false; } 
		$filter = new Ayoola_Filter_Name();
		$values['object_name'] = strtolower( $filter->filter( $values['class_name'] ) );
		if( $this->insertDb( $values ) ){ $this->setViewContent( '<p class="boxednews goodnews">Viewable Object Created</p>', true ); }
    } 
	// END OF CLASS
}
