...
//	var_export( $value );
	...
	public function createConfirmationForm( $name = null, $description = null, array $options = null  )
    {
		...
		$options =  array();
		$name =  $name ? : 'Continue...';
	//	$fieldset->addElement( array( 'name' => 'confirmation', 'label' => 'Do you want to continue?', 'description' =>  $description, 'type' => 'Select' ), $options );
...
//	var_export( $_POST );
	...
	//		var_export( $autoValues );
		//	var_export( $values );
			$this->getDbTable()->update( $values, $this->getIdentifier() );
		}
		catch( Ayoola_Dbase_Adapter_Xml_Table_Exception $e )
		{ 
		//	echo $e->getMessage();
...
//	var_export( array_shift( explode( ';', array_pop( explode( '/', $baseExt ) ) ) ) ); 
     ...
     */
    public function getGlobalValue( $name, $defaultValue = null )
    {		
		$this->fakeValues = $this->fakeValues ? : $this->getParameter( 'fake_values' );
