	...
	public function createForm( $submitValue, $legend = null, Array $values = null )
    {
		...
		if( is_array( Ayoola_Form::getGlobalValue( 'components' ) ) && in_array( 'modules', Ayoola_Form::getGlobalValue( 'components' ) ) )
		{
			...
			//	$options = Ayoola_Doc::getFiles( Ayoola_Application::getDomainSettings( APPLICATION_PATH ) . '/modules', array( 'return_directories' => true ) );  
				$directory = Ayoola_Application::getDomainSettings( APPLICATION_PATH ) . '/modules';  
		...
		{
 			$directory = Ayoola_Application::getDomainSettings( APPLICATION_PATH ) . '/databases';  
		//	var_export( $options );
			try
			{
		...
		{
 			$directory = Ayoola_Application::getDomainSettings( APPLICATION_PATH ) . '/documents';  
		//	var_export( $options );
			try
			{
				$options = Ayoola_Doc::getFilesRecursive( $directory ); 
		...
		if( is_array( Ayoola_Form::getGlobalValue( 'components' ) ) && in_array( 'plugins', Ayoola_Form::getGlobalValue( 'components' ) ) )
		{
 			$directory = Ayoola_Application::getDomainSettings( APPLICATION_PATH ) . '/plugins';  
		//	var_export( $options );
			try
			{
				$options = Ayoola_Doc::getFilesRecursive( $directory ); 
			...
			ksort( $files );
			$fieldset->addElement( array( 'name' => 'plugins', 'required' => 'required', 'label' => 'Select the plugins', 'type' => 'SelectMultiple', 'value' => @$values['plugins'] ), $files );
			if( $files ) 
			{
				$fieldset->addRequirement( 'plugins', array( 'ArrayKeys' => $files + array( 'badnews' => 'Please select plugins you want to include in the extension' )  ) );
			}
		...
		if( is_array( Ayoola_Form::getGlobalValue( 'components' ) ) && in_array( 'pages', Ayoola_Form::getGlobalValue( 'components' ) ) )
		{
			...
			ksort( $option );
			$fieldset->addElement( array( 'name' => 'pages', 'required' => 'required', 'label' => 'Select the pages. <a href="/object/name/Ayoola_Page_List/" target="_blank">Pages...</a>', 'type' => 'SelectMultiple', 'value' => @$values['pages'] ), $option );
			if( $option )     
			{
		...
		if( is_array( Ayoola_Form::getGlobalValue( 'components' ) ) && in_array( 'templates', Ayoola_Form::getGlobalValue( 'components' ) ) )
		{
			...
			$option = $filter->filter( $option );
			ksort( $option );
			$fieldset->addElement( array( 'name' => 'templates', 'required' => 'required', 'label' => 'Select the layout templates <a href="/object/name/Ayoola_Page_Layout_List/" target="_blank">Layouts...</a>', 'type' => 'SelectMultiple', 'value' => @$values['templates'] ), $option );
			if( $option ) 
			{
				$fieldset->addRequirement( 'templates', array( 'ArrayKeys' => $option + array( 'badnews' => 'Please select templates you want to include in the extension' )  ) );
			}
