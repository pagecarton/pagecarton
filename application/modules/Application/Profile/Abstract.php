...
abstract class Application_Profile_Abstract extends Ayoola_Abstract_Table
{
    ...
    {
		return self::getFolder() . DS . strtolower( implode( DS, str_split( $url, 2 ) ) ) . '.profile';
	}
    ...
    {
		$dir = self::getProfilePath( $values['profile_url'] );
		Ayoola_Doc::createDirectory( dirname( $dir ) );
		return file_put_contents( $dir, '<?php return ' . var_export( $values, true ) . ';' );
	}
			...
			(
				'
					ayoola.addShowProfileUrl = function( target )
					{
						...
						if( a )
						{
							element.innerHTML = "<span class=\'normalnews\'>The profile URL will be: <a href=\'/" + target.value + "\'>http://' . Ayoola_Page::getDefaultDomain() . '/" + target.value + "</a></span>";
						}  
						else
						{
