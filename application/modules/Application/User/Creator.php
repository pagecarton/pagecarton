...
class Application_User_Creator extends Application_User_Abstract 
{
    ...
    {
		$this->createForm( 'Create Account' );
...
/* 			$this->getForm()->setBadnews( 'A user is currently logged in. To sign up for a new account, you must first <a href="' . Ayoola_Application::getUrlPrefix() . '/accounts/signout/?previous_url=' . Ayoola_Page::getCurrentUrl() . '">Sign Out</a> of the existing account.' );
 ...
 */				//	exit( 'Application can not run while installation is in progress.' ); 
	...
	//	var_export( $saved );
		...
		{
			if( ! Ayoola_Access_Login::localLogin( $values ) )
			{
				$response = Ayoola_Access_Login::apiLogin( $values );
			}
		//	var_export( $response );
