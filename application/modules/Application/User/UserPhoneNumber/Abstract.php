...
abstract class Application_User_UserPhoneNumber_Abstract extends Ayoola_Abstract_Table
{
	...
	//	$form->setSubmitButton( $submitValue );
		...
		//	var_export( $defaultCountryValue );
			$fieldset->addElement( array( 'name' => 'country_id', 'label' => 'Country Code', 'style' => 'min-width:10%;max-width:25%;display:inline;margin-right:0;', 'type' => 'Select', 'value' => $defaultCountryValue ), array( 0 => 'Choose country code' ) + $options ); 
			$fieldset->addRequirement( 'country_id', array( 'InArray' => array_keys( $options )  ) );
