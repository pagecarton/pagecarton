...
/* 		
		...
		$requirements = array_map( 'trim', explode( ',', @$values['subscription_requirements'] ) );
	//	var_export( $values );
	...
	//		continue;
			...
			if( is_array( @self::$_requirementOptions[$each]['parameters'] ) )
			{
				$parameters += self::$_requirementOptions[$each]['parameters'];
...
//		var_export( $newCart );
		...
		$mailInfo['body'] = 'Someone added an item into their shopping cart "' . var_export( $newCart, true ) . '". 
