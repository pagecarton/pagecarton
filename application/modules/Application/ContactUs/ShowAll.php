...
class Application_ContactUs_ShowAll extends Application_ContactUs_Abstract
{
	...
	protected function init()
    {
		try
		{
			if( ! $this->getDbData() ){ return $this->setViewContent( '<div class="noRecord">No one has left a message on the application yet.</div>', true ); }
	...
	//	krsort( $values );
		...
		$columnNode = $this->_xml->createHTMLElement( 'td', '<div class="head">Contact Messages</div>' );
		$row->appendChild( $columnNode );
		$link = $this->_xml->createElement( 'a', 'Show All' );
		$link->setAttribute( 'href', '/tools/classplayer/get/object_name/Application_ContactUs_List/' );
		...
		foreach( $values as $data )
		{
			...
			$row->appendChild( $columnNode );
			$link = $this->_xml->createElement( 'a', $data['contactus_subject'] );
			$link->setAttribute( 'href', '/tools/classplayer/get/object_name/Application_ContactUs_View/contactus_id/' . strtolower( $data['contactus_id'] ) . '/' );
			$link->setAttribute( 'rel', 'spotlight;' );
			$columnNode->appendChild( $link );
