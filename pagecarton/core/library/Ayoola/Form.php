...
//			$form .= "</div>\n";    
		...
		$form .= $fieldset->allowDuplication ? "<a class='pc-btn pc-btn-small' href='javascript:' title='" . ( @$fieldset->duplicationData['add'] ? : "Remove this fieldset" ) . "' onClick='confirm( \"Delete all the elements in these fieldset?\") ? this.parentNode.parentNode.removeChild( this.parentNode ) : null; ayoola.div.refreshVisibleCounter(\"" . @$fieldset->duplicationData['counter'] . "\");'>" . ( @$fieldset->duplicationData['remove'] ? : " - " ) . "</a></div>\n" : null; 
	//	var_export( $div );  
	//	var_export( $this->_values );
