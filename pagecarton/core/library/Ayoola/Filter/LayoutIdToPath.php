	...
	class Ayoola_Filter_LayoutIdToPath implements Ayoola_Filter_Interface
	{
		...
		*/
        ...
        //    var_export( Ayoola_Loader::getFullPath( $themeDir ) );
			if( $path = Ayoola_Loader::getFullPath( $themeDir, $options ) )
			{
				$fileC[] = $path;
            ...
            elseif( $path = Ayoola_Loader::getFullPath( $themeDir . 'auto', ! empty( $options['always_blacklist'] ) ? $options : null ) )
            {
            //    var_export( $path );
				$fileC[] = $path;
            }
			...
			else
			{
                ...
                )
				{
					...
					$PAGE_INCLUDE_FILE = $PAGE_INCLUDE_FILE ?
						: Ayoola_Loader::getFullPath( 'documents/layout/' . $themeName . '/theme/includeauto', array( 'prioritize_my_copy' => true ) );
				}
				if( 
                    stripos( $this->_pageInfo['url'], '/default-layout' ) === 0 
                    ...
                    OR Ayoola_Loader::getFullPath( $mPaths, array( 'prioritize_my_copy' => true ) )   )
				{
                    ...
                    //  autogenerated theme file
					$PAGE_TEMPLATE_FILE = $PAGE_TEMPLATE_FILE ? : Ayoola_Loader::getFullPath( 'documents/layout/' . $themeName . '/theme/templateauto', array( 'prioritize_my_copy' => true ) ); 
				}
