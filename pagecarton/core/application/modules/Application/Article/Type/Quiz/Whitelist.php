...
class Application_Article_Type_Quiz_Whitelist extends Application_Article_Type_Quiz
{
	...
	//		var_export( $data );
			...
			if( ! self::isOwner( $data['user_id'] ) && ! self::isAllowedToEdit( $data ) && ! self::hasPriviledge( $articleSettings['allowed_editors'] ) && Ayoola_Application::getUserInfo( 'username' ) !== strtolower( $data['username'] ) ){ return false; }
