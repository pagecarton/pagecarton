...
abstract class Application_User_UserLocation_Abstract extends Ayoola_Abstract_Table
{
	...
	//		if( $this->getGlobalValue( 'city' ) && $this->getGlobalValue( 'province_id' ) )
		...
		//	Province ID
			$fieldset->addElement( array( 'name' => 'province', 'class' => 'geotext[state-long]', 'label' => 'State, Province or Region', 'e.g.' => 'e.g. Lagos', 'description' => 'Please select a state', 'type' => 'InputText', 'value' => @$values['province'] ), array( 0 => 'Please select...' ) + ( is_array( $listProvince ) ? $listProvince : array()  ) );    
	//		$listProvince ? $fieldset->addRequirement( 'province_id', array( 'InArray' => array_keys( $listProvince )  ) ) : null;
//var_export( $listProvince );	
		...
		//	$fieldset->addRequirement( 'zip', array( 'WordCount' => array( 3, 10 ) ) );
			if( $listCountry ) 
			{
				$defaultCountryValue = ( @$values['country'] ? : $this->getObjectStorage( 'country_id_value' )->retrieve() ) ? : Application_Settings_CompanyInfo::getSettings( 'CompanyInformation', 'country' );
