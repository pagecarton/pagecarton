...
//ayoola.files.loadJsObjectCss( 'webReferenceDragNDrop' );
//	Object Begins
var Demos       = []; 
var nDemos      = 8;
...
// Demo 0 variables
var renewableDragContainers = new Object;
var DragDrops   = [];
var curTarget   = null;
var lastTarget  = null;
var dragHelper  = null;
...
/* 	var cDrag        = DragDrops.length;
	DragDrops[cDrag] = [];
 */	var cDrag        = 0; //	drop anywhere
	DragDrops[cDrag] = DragDrops[cDrag] ? DragDrops[cDrag] : [];
	...
	*/
	for(var i=0; i<arguments.length; i++){
		...
		if( ! cObj || cObj.nodeName == '#text' ){	continue; }
	//	alert( cObj );
		...
		*/
		for(var j=0; j<cObj.childNodes.length; j++){
			...
			if(cObj.childNodes[j].name=='add_a_new_item_to_parent_section') continue;
			//	alert( cObj.childNodes[j].className );
			
			cObj.childNodes[j].setAttribute ? cObj.childNodes[j].setAttribute('DragObj', cDrag) : null;
			 
			//	add delete button to sectional objects
	//		if( cObj.id != "viewable_objects" )
			{
		//		var deleteButton = ayoola.div.getDelete( cObj.childNodes[j] );
		//		deleteButton.title = 'Delete Object';
		//		deleteButton.innerHTML = ' - ';
		//		cObj.childNodes[j].appendChild( deleteButton );
			}
/* 			var child = cObj.childNodes[j];
			for( var a = 0; a < child.childNodes.length; a++ )
			{
				if( child.childNodes[a].nodeName == '#text' ){ continue; }
				if( child.childNodes[a].dataset.parameter_name != 'view' ){ continue; };
				ayoola.div.makeEditable( child.childNodes[a] );
			}
 */		}
	}
}

//	Renewable Dragbox container allows element to replenish elements dragged away from them
function createRenewableDragContainer()
{
	for(var i=0; i<arguments.length; i++)		
	{
		...
		CreateDragContainer( element );
		renewableDragContainers[element.id] = element;
	}
